...
(xor
 ...
 (call %init_peer_id% ("errorHandlingSrv" "error") [%last_error%])
            ...
            .configHandler((h) => {
                ...
                h.on('getDataSrv', 'currentUserName', () => {return currentUserName;});
...
export async function addEntry(client: FluenceClient, entry: string, selfPeerId: string): Promise<{entry_id:number;err_msg:string;ret_code:number}> {
    let request;
...
h.on('getDataSrv', 'selfPeerId', () => {return selfPeerId;});
                h.onEvent('callbackSrv', 'response', (args) => {
  const [res] = args;
  resolve(res);
});
